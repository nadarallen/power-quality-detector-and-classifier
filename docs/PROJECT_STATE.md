...
| **Acquisition Adapters** | `dsp/acquisition_adapter.py` | Abstract `AcquisitionAdapter`, `HardwareAdapter`, `DAQAdapter`, `SerialAdapter`, `NetworkAdapter`, `PQMeterAdapter`, `MockHardwareAdapter`, `SimulationAdapter`, `CSVReplayAdapter`, and `AcquisitionState` management (`CONNECTED`, `DISCONNECTED`, `ACQUIRING`, `ERROR`, `SYNC_ERROR`, `INVALID_SIGNAL`). |
...
| **Streaming Pipeline** | `pipeline/realtime_pipeline.py` | Continuous streaming engine connecting acquisition adapter, signal validation, ring buffer sliding windowing, event engine, genuine spectral telemetry broadcasting, and persistence callbacks. |
...
| **Web UI & Oscilloscope** | `web/index.html`<br>`web/app.js`<br>`web/styles.css` | Retro laboratory CRT oscilloscope UI, 3-phase multi-trace overlay rendering (`#f1e05a`, `#58a6ff`, `#ff7b72`), genuine Goertzel harmonic bar spectrum, target channel disturbance injection console, live grid bus metrics bar, Server-Sent Events (SSE) telemetry streaming, and client-side neural network inference. |
| **Hardware & Protocol Specs** | `docs/ACQUISITION_REQUIREMENTS.md`<br>`docs/HARDWARE_EVALUATION.md`<br>`docs/LATENCY_BENCHMARK.md`<br>`docs/REAL_HARDWARE_VALIDATION.md` | Specifications for real acquisition hardware selection, isolation requirements, latency profiling ($31.8\text{ ms}$ 3-phase end-to-end), and physical validation protocol. |
| **Automated Tests** | `tests/` (108 test cases) | Rigorous physical, standards, firmware parity, 3-phase real-time pipeline, ring buffer, multi-window merging, hardware acquisition/calibration/saturation, hardware state transitions, and REST API test suite (107 passed, 1 skipped cleanly). |
...
- **Passed:** 107
- **Failed:** 0
- **Skipped:** 1 (physical hardware test cleanly skipped when `LIVE_HARDWARE_TESTS=1` is not set)
...
- `tests/test_hardware_acquisition.py`: 12 tests (Calibration counts-to-Volts-to-pu, saturation rail detection, ThreePhaseCalibration profile, MockHardwareAdapter emission, sequence gap dropped samples, clipping detection, canonical .json/.npz roundtrip, end-to-end mock hardware to MLP PQEvent, adapter interface contracts, state transitions, rich spectral telemetry, and live-hardware test skipping).
